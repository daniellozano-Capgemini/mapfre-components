...
import { PipeFormatOptions } from '../../pipes/currency-formatter.pipe';
...
export class MapfreInputComponent
  ...
  implements OnInit
{
  ...
  ngAfterViewInit(): void {
    if (this.maxlength)
      this.inputEl.nativeElement.setAttribute('maxlength', this.maxlength);
    if (this.minlength)
      this.inputEl.nativeElement.setAttribute('minlength', this.minlength);
  }
  ...
  onInput($event: Event) {
    ...
    if (this.type === 'currency') {
      ...
      this.previusValue =
        this.coma !== ''
          ? this.formatearPositiveNumToThousands(this.numero) +
            this.coma +
            this.decimal
          : this.formatearPositiveNumToThousands(this.numero);
  ...
  resetAndSetNum(e) {
    ...
    if (e.target['value'].includes(',')) {
      ...
      this.decimal = e.target['value'].split(',')[1]
        ? e.target['value'].split(',')[1].trim()
        : '';
      this.decimal =
        this.decimal.length > 1
          ? Array.from(this.decimal)[0] + Array.from(this.decimal)[1]
          : this.decimal;
    }
  ...
  autoCompleteDecimals(
    num: string,
    showCurrencyInInput: boolean,
    coin: string
  ): string {
    let value = num;
    const valueHaveComma =
      value.includes(',') && value.split(',')[1] ? value.split(',')[1] : '';
    const autoCompleteValue =
      valueHaveComma.length === 0
        ? '00'
        : valueHaveComma.length === 1
        ? valueHaveComma + '0'
        : valueHaveComma;
    value =
      valueHaveComma.length > 0
        ? value.split(',')[0] + ',' + autoCompleteValue
        : value.includes(',')
  ...
  onDataChanged(event) {
    event.value || event.value === ''
      ? this.dataChanged.emit(event.value)
